...
class Bot {
     ...
     */
    async uploadAttachmentToS3(attachment, customFilename) {
        if (!this.s3Uploader) {
            return null;
        }
        try {
            ...
            // Upload to S3
            const result = await this.s3Uploader.uploadFile(buffer, attachment.name || 'unknown', customFilename, attachment.contentType || undefined);
