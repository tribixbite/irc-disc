...
exports.TEST_HACK_CHANNEL = void 0;
...
const persistence_wrapper_js_1 = require("./persistence-wrapper.js");
...
const mention_detector_1 = require("./mention-detector");
const status_notifications_1 = require("./status-notifications");
const irc_user_manager_1 = require("./irc-user-manager");
...
process.on('unhandledRejection', (reason, promise) => {
    logger_1.logger.error('CRITICAL: Unhandled Promise Rejection at:', promise);
    logger_1.logger.error('Rejection reason:', reason);
});
// Polyfill for deprecated util.log (removed in Node.js 24)
// The irc-upd library still uses it for debug logging
if (!util_1.default.log) {
    util_1.default.log = function (...args) {
...
class Bot {
    ...
    pmThreadPrefix;
    pmAutoArchive;
    pmThreads; // ircNick -> threadId mapping with LRU eviction
    // Persistence service
    persistence;
    ...
    ircConnected = false;
    ircRegistered = false;
    ircReconnecting = false; // Guard against concurrent reconnections
    lastIRCActivity = Date.now();
    ircHealthCheckInterval;
    constructor(options) {
        ...
        (0, validators_1.validateChannelMapping)(options.channelMapping);
        this.discord = new discord_js_1.default.Client({
            retryLimit: 3,
            intents: [
                discord_js_1.Intents.FLAGS.GUILDS,
                discord_js_1.Intents.FLAGS.GUILD_MESSAGES,
                discord_js_1.Intents.FLAGS.GUILD_MESSAGE_REACTIONS,
                discord_js_1.Intents.FLAGS.GUILD_MEMBERS, // Required for member cache (nicknames, avatars)
                discord_js_1.Intents.FLAGS.MESSAGE_CONTENT // Required for message.content access (Discord.js v13+)
            ],
            partials: ['MESSAGE'], // Enable partial message support for edit/delete events
        });
        // Add unique instance ID for debugging
        this.discord._instanceId = Math.random().toString(36).substring(7);
        logger_1.logger.info(`[DIAGNOSTIC] Discord Client created with instance ID: ${this.discord._instanceId}`);
        this.server = options.server;
        this.nickname = options.nickname;
        this.ircOptions = options.ircOptions;
        // Initialize persistence service with configurable cleanup thresholds
        this.persistence = new persistence_wrapper_js_1.PersistenceService(options.dbPath, options.dbCleanupPMThreadDays, options.dbCleanupChannelUsersDays);
        ...
        logger_1.logger.debug('format.ircText from config:', JSON.stringify(this.format.ircText));
        this.formatIRCText = this.format.ircText || '<{$displayUsername}> {$text}';
        logger_1.logger.info(`Using IRC text format: ${this.formatIRCText}`);
        ...
        for (const [discordChan, ircChan] of Object.entries(options.channelMapping)) {
        ...
        const pmCacheSize = options.pmThreadCacheSize || 500;
        this.pmThreads = new lru_cache_1.LRUCache({
            max: pmCacheSize,
            ttl: 1000 * 60 * 60 * 24 * 7, // 7 days TTL
        });
        // Initialize message synchronization
        this.messageSync = new message_sync_1.MessageSynchronizer(this);
        // Initialize metrics collection
        try {
            this.metrics = new metrics_1.MetricsCollector(this.persistence);
        }
        catch (error) {
            logger_1.logger.warn('Failed to initialize metrics with persistence, using in-memory metrics:', error);
            this.metrics = new metrics_1.MetricsCollector(null);
        }
        ...
        if (metricsPort) {
            this.metricsServer = new metrics_server_1.MetricsServer(this.metrics, metricsPort);
        }
        // Initialize error recovery manager
        try {
            const recoveryConfig = options.recovery || {};
            this.recoveryManager = new recovery_manager_1.RecoveryManager(recoveryConfig);
            this.setupRecoveryHandlers();
        }
        catch (error) {
            logger_1.logger.warn('Failed to initialize recovery manager:', error);
        ...
        const s3Config = this.loadS3Config(options.s3);
        if (s3Config) {
            try {
                this.s3Uploader = new s3_uploader_1.S3Uploader(s3Config);
                // Test connection on initialization (async, fire and forget)
                void this.s3Uploader.testConnection().then(result => {
                    if (!result.success) {
                        logger_1.logger.error('S3 connection test failed, disabling S3 uploads:', result.error);
                        this.s3Uploader = undefined;
                    }
                });
            }
            catch (error) {
                logger_1.logger.error('Failed to initialize S3 uploader:', error);
                this.s3Uploader = undefined;
            }
        }
        // Initialize mention detection
        const mentionConfig = this.loadMentionConfig(options.mentions);
        this.mentionDetector = new mention_detector_1.MentionDetector(mentionConfig);
        ...
        const statusConfig = status_notifications_1.StatusNotificationManager.loadConfig(options);
        // Pass enabled=false to constructor to prevent spam (respects explicit config if set)
        const finalStatusConfig = {
            ...statusConfig,
            enabled: options.statusNotifications?.enabled ?? false
        };
        this.statusNotifications = new status_notifications_1.StatusNotificationManager(finalStatusConfig);
    }
    /**
     ...
     */
    async resolveViaGetent(hostname) {
        try {
            ...
            if (ipMatch && ipMatch[1]) {
                ...
                logger_1.logger.info(`✅ Successfully resolved ${hostname} to ${ipAddress} via ping.`);
                return ipAddress;
            }
            else {
                logger_1.logger.warn(`ping output did not contain IP for ${hostname}. Output: ${rawOutput.substring(0, 200)}. Falling back to hostname.`);
                return hostname; // Fallback
            }
        }
        catch (error) {
            logger_1.logger.error(`Error spawning ping process to resolve ${hostname}:`, error);
            return hostname; // Fallback on spawn error
        }
    }
    async connect() {
        logger_1.logger.debug('Connecting to IRC and Discord');
        ...
        if (!process.env.S3_CONFIG_ENCRYPTION_KEY) {
            ...
            if (dbEncryptionKey) {
                ...
                logger_1.logger.info('S3 encryption key loaded from database');
            }
        }
        ...
        try {
            logger_1.logger.info(`[DIAGNOSTIC] Logging in to Discord with client instance: ${this.discord._instanceId}`);
            await this.discord.login(this.discordToken);
            logger_1.logger.info(`[DIAGNOSTIC] Discord login promise resolved for instance: ${this.discord._instanceId}`);
            ...
            setInterval(() => {
                logger_1.logger.debug('[CANARY] Event loop is alive');
            }, 60000);
            logger_1.logger.info('[DIAGNOSTIC] Event loop canary started (logs every 60s at debug level)');
        }
        catch (error) {
            logger_1.logger.error('Discord login promise rejected:', error);
            throw error; // Re-throw to ensure the application fails fast
        }
        // Extract id and token from Webhook urls and connect.
        for (const [channel, url] of Object.entries((this.webhookOptions ?? {}))) {
        ...
        if (!Object.prototype.hasOwnProperty.call(ircOptions, 'encoding')) {
            if (irc_upd_1.default.canConvertEncoding()) {
                ircOptions.encoding = 'utf-8';
            }
            else {
                logger_1.logger.warn('Cannot convert message encoding; you may encounter corrupted characters with non-English text.\n' +
        ...
        setImmediate(() => {
            void (async () => {
                try {
                    logger_1.logger.info('Initializing IRC client in next event loop tick...');
                    ...
                    this.ircClient = new irc_upd_1.default.Client(ircServerAddress, this.nickname, enhancedOptions);
                    // Initialize IRC user manager
                    this.ircUserManager = new irc_user_manager_1.IRCUserManager(this.ircClient, {
                        enableWhois: false
                    });
                    // Attach IRC event listeners BEFORE connecting
                    this.attachIRCListeners();
                    logger_1.logger.info('IRC client constructed and listeners attached.');
                    ...
                    logger_1.logger.info(`Connecting to IRC server: ${ircServerAddress} (${this.server})`);
                    this.ircClient.connect(ircOptions.retryCount, () => {
                        // This callback fires upon successful registration to IRC server
                        logger_1.logger.info(`✅ Successfully connected and registered to IRC server: ${this.server}`);
                    });
                }
                catch (error) {
                    logger_1.logger.error('Failed to initialize IRC client:', error);
                }
            })();
        });
    ...
    setupRecoveryHandlers() {
        ...
        this.recoveryManager.on('attemptReconnection', (service, callback) => {
            void (async () => {
                try {
                    logger_1.logger.info(`Attempting to reconnect ${service}...`);
                    if (service === 'discord') {
                    ...
                    logger_1.logger.error(`Reconnection attempt failed for ${service}:`, error);
                    callback(false);
                }
            })();
        });
        ...
        this.recoveryManager.on('recoveryStarted', (service, error, maxRetries) => {
            logger_1.logger.warn(`🔄 Recovery started for ${service}: ${error.message}`);
        ...
        this.recoveryManager.on('recoveryFailed', (service, error) => {
            logger_1.logger.error(`❌ Recovery failed for ${service}: ${error.message}`);
            this.metrics.recordError();
        });
        this.recoveryManager.on('circuitBreakerTripped', (service, health) => {
            logger_1.logger.error(`🚫 Circuit breaker tripped for ${service} after ${health.consecutiveFailures} failures`);
        });
        ...
        this.recoveryManager.on('serviceSilent', (service, health) => {
            logger_1.logger.warn(`⚠️ ${service} has been silent for ${Date.now() - health.lastSuccessful}ms`);
        });
    }
    /**
    ...
    async reconnectDiscord() {
        try {
            logger_1.logger.info('Reconnecting Discord client...');
            ...
            this.discord = new discord_js_1.default.Client({
                ...
                intents: [
                    ...
                    discord_js_1.default.Intents.FLAGS.GUILD_MEMBERS, // Required for member cache
                    discord_js_1.default.Intents.FLAGS.MESSAGE_CONTENT // Required for message.content access
                ],
                partials: ['MESSAGE']
            });
            ...
            logger_1.logger.info('Discord reconnection successful');
            this.metrics.recordDiscordReconnect();
            this.recoveryManager.recordSuccess('discord');
            return true;
        }
        catch (error) {
            logger_1.logger.error('Discord reconnection failed:', error);
    ...
    async reconnectIRC() {
        ...
        if (this.ircReconnecting) {
            logger_1.logger.debug('IRC reconnection already in progress, skipping duplicate attempt');
            return false;
        }
        this.ircReconnecting = true;
        try {
            logger_1.logger.info('Reconnecting IRC client...');
            ...
            logger_1.logger.info(`Resolving IRC server hostname: ${this.server}`);
            const ircServerAddress = await this.resolveViaGetent(this.server);
            ...
            logger_1.logger.info(`✅ IRC reconnection successful to ${ircServerAddress} (${this.server})`);
            this.metrics.recordIRCReconnect();
            this.recoveryManager.recordSuccess('irc');
            return true;
        }
        catch (error) {
            logger_1.logger.error('❌ IRC reconnection failed:', error);
    ...
    attachDiscordListeners() {
        logger_1.logger.info('[DIAGNOSTIC] attachDiscordListeners() called - attaching event handlers');
        logger_1.logger.info(`[DIAGNOSTIC] Attaching listeners to client instance: ${this.discord._instanceId}`);
        ...
        this.discord.on('raw', (packet) => {
            if (packet.t === 'MESSAGE_CREATE') {
                logger_1.logger.info(`[RAW] MESSAGE_CREATE gateway packet received from Discord!`);
                logger_1.logger.info(`[RAW] Packet channel_id: ${packet.d?.channel_id}, author: ${packet.d?.author?.username}`);
            }
        });
        ...
        this.discord.on('debug', (info) => {
            logger_1.logger.info(`[DJS DEBUG] ${info}`);
        });
        this.discord.on('warn', (warning) => {
            logger_1.logger.warn(`[DJS WARN] ${warning}`);
        });
        this.discord.on('ready', async () => {
            logger_1.logger.info('Connected to Discord');
            logger_1.logger.info(`[DIAGNOSTIC] ready event fired on client instance: ${this.discord._instanceId}`);
            // Register slash commands when bot is ready
            await (0, slash_commands_1.registerSlashCommands)(this);
        ...
        this.discord.on('error', (error) => {
            logger_1.logger.error('Received error event from Discord', error);
            this.metrics.recordConnectionError();
            this.recoveryManager.recordFailure('discord', error);
        });
        ...
        this.discord.on('reconnecting', () => {
            logger_1.logger.info('Discord client attempting to reconnect');
        });
        this.discord.on('warn', (warning) => {
            logger_1.logger.warn('Received warn event from Discord', warning);
        });
        this.discord.on('messageCreate', (message) => {
            logger_1.logger.info(`[EVENT] messageCreate fired on client instance: ${message.client._instanceId}`);
            logger_1.logger.info(`[EVENT] Message details - Channel: ${message.channel.id}, Author: ${message.author.tag}, Content: ${message.content?.substring(0, 50) || '(no content)'}`);
            ...
            if (message.channel &&
                ...
                this.handleDiscordPrivateMessage(message).catch((error) => {
                    logger_1.logger.error('Error handling Discord PM:', error);
                });
                return;
            }
            // Handle regular channel messages with proper error handling
            this.sendToIRC(message).catch((error) => {
                logger_1.logger.error('Error sending Discord message to IRC:', error);
            });
        });
        logger_1.logger.info('[DIAGNOSTIC] messageCreate listener attached');
        // Handle slash command interactions
        this.discord.on('interactionCreate', async (interaction) => {
            if (interaction.isCommand()) {
                await (0, slash_commands_1.handleSlashCommand)(interaction, this);
            }
            else if (interaction.isButton()) {
                await (0, slash_commands_1.handleButtonInteraction)(interaction, this);
            }
        });
    ...
    attachIRCListeners() {
        this.ircClient.on('registered', (message) => {
            logger_1.logger.info('✅ Connected and registered to IRC');
            logger_1.logger.debug('Registered event: ', message);
            // Update connection state
        ...
        this.ircClient.on('error', (error) => {
            logger_1.logger.error('❌ Received error event from IRC', error);
        ...
        this.ircClient.on('abort', () => {
            logger_1.logger.warn('❌ IRC connection aborted');
            // Update connection state
            this.ircConnected = false;
            this.ircRegistered = false;
            // Send IRC disconnected notification
        ...
        this.ircClient.on('close', () => {
            logger_1.logger.warn('❌ IRC connection closed');
            // Update connection state
            this.ircConnected = false;
            this.ircRegistered = false;
            // Send IRC disconnected notification
        ...
        this.ircClient.on('netError', (error) => {
            logger_1.logger.error('❌ IRC network error:', error);
            // Update connection state
            this.ircConnected = false;
            this.ircRegistered = false;
            // Send IRC disconnected notification
        ...
        this.ircClient.on('message', (author, channel, text) => {
            ...
            this.sendToDiscord(author, channel, text).catch((error) => {
                logger_1.logger.error('Error sending IRC message to Discord:', error);
            });
        });
        ...
        this.ircClient.on('notice', (author, to, text) => {
            ...
            this.sendToDiscord(author, to, `*${text}*`).catch((error) => {
                logger_1.logger.error('Error sending IRC notice to Discord:', error);
            });
        });
        ...
        this.ircClient.on('nick', (oldNick, newNick, channels) => {
            (async () => {
                ...
                this.updatePmThreadForNickChange(oldNick, newNick).catch((error) => {
                    logger_1.logger.error('Error updating PM thread for nick change:', error);
                });
                ...
                for (const channelName of channels) {
                    ...
                    if (this.channelUsers[channel]) {
                        if (this.channelUsers[channel].has(oldNick)) {
                            ...
                            await this.sendExactToDiscord(channel, `*${oldNick}* is now known as ${newNick}`);
                            ...
                            this.saveChannelUsersToPersistence(channel).catch((error) => {
                                logger_1.logger.error('Failed to save channel users after nick change:', error);
                            });
                        }
                    }
                    else {
                        logger_1.logger.warn(`No channelUsers found for ${channel} when ${oldNick} changed.`);
                    }
                }
            })().catch((error) => {
                logger_1.logger.error('Error handling IRC nick change:', error);
            });
        });
        // Handle IRC joins with proper async error handling
        this.ircClient.on('join', (channelName, nick) => {
            (async () => {
                logger_1.logger.debug('Received join:', channelName, nick);
                const channel = channelName.toLowerCase();
                ...
                if (!isBotEvent) {
                    ...
                    // Save updated channel users to persistence (don't await to avoid blocking)
                    this.saveChannelUsersToPersistence(channel).catch((error) => {
                        logger_1.logger.error('Failed to save channel users after join:', error);
                    });
                }
                // Send join notification via status notification manager
                const discordChannel = this.findDiscordChannel(channel);
                if (discordChannel && isTextChannel(discordChannel)) {
                    const sent = await this.statusNotifications.sendJoinNotification(nick, channelName, discordChannel, isBotEvent);
                ...
                logger_1.logger.error('Error handling IRC join:', error);
            });
        });
        // Handle IRC parts with proper async error handling
        this.ircClient.on('part', (channelName, nick, reason) => {
            (async () => {
                logger_1.logger.debug('Received part:', channelName, nick, reason);
                const channel = channelName.toLowerCase();
                const isBotEvent = nick === this.ircClient.nick;
                // Handle bot parting - remove channel user tracking
                if (isBotEvent) {
                    logger_1.logger.debug('Deleting channelUsers as bot parted:', channel);
                ...
                if (this.channelUsers[channel]) {
                    ...
                    // Save updated channel users to persistence (don't await to avoid blocking)
                    this.saveChannelUsersToPersistence(channel).catch((error) => {
                        logger_1.logger.error('Failed to save channel users after part:', error);
                    });
                }
                else {
                    logger_1.logger.warn(`No channelUsers found for ${channel} when ${nick} parted.`);
                }
                // Send leave notification via status notification manager
                const discordChannel = this.findDiscordChannel(channel);
                if (discordChannel && isTextChannel(discordChannel)) {
                    const sent = await this.statusNotifications.sendLeaveNotification(nick, channelName, reason || '', discordChannel, isBotEvent);
                    // Fallback to legacy system if status notifications are disabled
                    if (!sent && this.ircStatusNotices) {
                ...
                logger_1.logger.error('Error handling IRC part:', error);
            });
        });
        // Handle IRC quits with proper async error handling
        this.ircClient.on('quit', (nick, reason, channels) => {
            (async () => {
                logger_1.logger.debug('Received quit:', nick, channels);
                const isBotEvent = nick === this.ircClient.nick;
                if (isBotEvent)
                    return; // Ignore bot's own quit events
                const processedChannels = new Set();
                for (const channelName of channels) {
                    const channel = channelName.toLowerCase();
                    // Update channel users tracking
                    if (!this.channelUsers[channel]) {
                        logger_1.logger.warn(`No channelUsers found for ${channel} when ${nick} quit, ignoring.`);
                        continue;
                    }
                    if (!this.channelUsers[channel].delete(nick))
                        continue;
                    // Send quit notification via status notification manager (only once per user)
                    const discordChannel = this.findDiscordChannel(channel);
                    if (discordChannel && isTextChannel(discordChannel) && !processedChannels.has(channel)) {
                        processedChannels.add(channel);
                        const sent = await this.statusNotifications.sendQuitNotification(nick, reason || '', discordChannel, isBotEvent);
                        // Fallback to legacy system if status notifications are disabled
                        if (!sent && this.ircStatusNotices) {
                            await this.sendExactToDiscord(channel, `*${nick}* has quit (${reason})`);
                        }
                    }
                }
            })().catch((error) => {
                logger_1.logger.error('Error handling IRC quit:', error);
            });
        });
        this.ircClient.on('names', (channelName, nicks) => {
            logger_1.logger.debug('Received names:', channelName, nicks);
            if (!this.ircStatusNotices)
                return;
            const channel = channelName.toLowerCase();
            ...
            this.saveChannelUsersToPersistence(channel).catch((error) => {
                logger_1.logger.error('Failed to save initial channel users:', error);
            });
        });
        // Handle IRC actions with proper async error handling
        this.ircClient.on('action', (author, to, text) => {
            this.sendToDiscord(author, to, `_${text}_`).catch((error) => {
                logger_1.logger.error('Error sending IRC action to Discord:', error);
            });
        });
        this.ircClient.on('invite', (channel, from) => {
            logger_1.logger.debug('Received invite:', channel, from);
            if (!this.invertedMapping[channel]) {
                logger_1.logger.debug('Channel not found in config, not joining:', channel);
            }
            else {
                this.ircClient.join(channel);
                logger_1.logger.debug('Joining channel:', channel);
            }
        });
        if (logger_1.logger.level === 'debug') {
            this.discord.on('debug', (message) => {
                logger_1.logger.debug('Received debug event from Discord', message);
            });
        }
    }
    ...
    async sendToIRC(message) {
        ...
        if (!this.isIRCConnected()) {
            logger_1.logger.debug(`Skipping rate limit check for ${author.username} - IRC connection is down, message won't be sent`);
            ...
            const rateLimitResult = this.rateLimiter.checkMessage(author.id, author.username, messageContent);
            if (rateLimitResult) {
                logger_1.logger.warn(`Message from ${author.username} (${author.id}) blocked by rate limiter: ${rateLimitResult}`);
                ...
                try {
                    ...
                    logger_1.logger.debug(`Could not send rate limit warning DM to ${author.username}:`, error);
                }
        ...
        if (!this.isIRCConnected()) {
            logger_1.logger.warn(`Message from ${author.username} dropped - IRC not connected`);
            ...
            try {
                ...
                logger_1.logger.debug(`Could not reply to ${author.username} about dropped message:`, error);
            }
        ...
        logger_1.logger.debug('Channel Mapping', channelName, this.channelMapping[channelName]);
        if (ircChannel) {
            ...
            if (this.ircNickColor) {
                ...
                displayUsername = irc_upd_1.default.colors.wrap(this.ircNickColors[colorIndex], displayUsername);
            }
            const patternMap = {
                author: nickname,
                nickname,
            ...
            if (this.isCommandMessage(text)) {
                ...
                logger_1.logger.debug('Sending command message to IRC', ircChannel, text);
                ...
                if (text !== '') {
                    ...
                    for (const orig of sentences) {
                        let sentence = (0, formatting_1.formatFromDiscordToIRC)(orig);
                        if (sentence) {
                            patternMap.text = sentence;
                            sentence = Bot.substitutePattern(this.formatIRCText, patternMap);
                            logger_1.logger.debug('Sending message to IRC', ircChannel, sentence);
                    ...
                    if (sentences.some(s => (0, formatting_1.formatFromDiscordToIRC)(s))) {
                        this.metrics.recordDiscordToIRC(author.id, ircChannel);
                        // Mark Discord as active (message sent to IRC)
                        this.recoveryManager.recordSuccess('discord');
                    }
                    ...
                    for (const [, attachment] of message.attachments) {
                        ...
                        if (this.s3Uploader) {
                            try {
                                ...
                                if (s3Url) {
                                    attachmentURL = s3Url;
                                    logger_1.logger.debug('Using S3 URL for attachment:', attachment.name);
                                }
                                else {
                                    logger_1.logger.debug('S3 upload failed, using Discord URL for attachment:', attachment.name);
                                }
                            }
                            catch (error) {
                                logger_1.logger.warn('S3 upload error, using Discord URL:', error);
                            }
                        }
                        patternMap.attachmentURL = attachmentURL;
                        const urlMessage = Bot.substitutePattern(this.formatURLAttachment, patternMap);
                        logger_1.logger.debug('Sending attachment URL to IRC', ircChannel, urlMessage);
    ...
    findDiscordChannel(ircChannel) {
        ...
        if (discordChannelName) {
            ...
            if (!discordChannel) {
                logger_1.logger.info("Tried to send a message to a channel the bot isn't in: ", discordChannelName);
                return null;
            }
    ...
    async sendToDiscord(author, channel, text) {
        ...
        const rateLimitResult = this.rateLimiter.checkMessage(`irc:${author}`, // Use IRC nickname with prefix to distinguish from Discord IDs
        author, text);
        if (rateLimitResult) {
            logger_1.logger.warn(`Message from IRC user ${author} blocked by rate limiter: ${rateLimitResult}`);
            ...
            try {
                ...
                logger_1.logger.debug(`Could not send rate limit warning PM to ${author}:`, error);
            }
            return; // Block the message
        }
        // Convert text formatting (bold, italics, underscore)
        const withFormat = (0, formatting_1.formatFromIRCToDiscord)(text);
        const patternMap = {
            author,
            nickname: author,
            ...
            ircChannel: channel,
            side: undefined,
            withMentions: undefined,
            withFilteredMentions: undefined,
        };
        if (this.isCommandMessage(text)) {
            patternMap.side = 'IRC';
            logger_1.logger.debug('Sending command message to Discord', `#${discordChannel.name}`, text);
            if (this.formatCommandPrelude) {
                const prelude = Bot.substitutePattern(this.formatCommandPrelude, patternMap);
        ...
        const processedText = withFormat
            ...
            const user = guild.members.cache.find((x) => Bot.caseComp(x.user.username, username) &&
        ...
        const mentionResult = this.mentionDetector.detectMentions(processedText, guild, author, Array.from(guild.members.cache.values()));
        const withMentions = mentionResult.textWithMentions;
        // Webhooks first
        const webhook = this.findWebhook(channel);
        if (webhook) {
            logger_1.logger.debug('Sending message to Discord via webhook', withMentions, channel, '->', `#${discordChannel.name}`);
            const permissions = discordChannel.permissionsFor(this.discord.user);
            let canPingEveryone = false;
            if (permissions) {
                canPingEveryone = permissions.has(discord_js_1.default.Permissions.FLAGS.MENTION_EVERYONE);
            }
        ...
        patternMap.withFilteredMentions = withMentions.replace(/@(here|everyone)/gi, (match, part) => `ම${part}`);
        // Add bold formatting:
        ...
        logger_1.logger.debug('Sending message to Discord', withAuthor, channel, '->', `#${discordChannel.name}`);
    ...
    async sendExactToDiscord(channel, text) {
        const discordChannel = this.findDiscordChannel(channel);
        if (!discordChannel)
            return;
        logger_1.logger.debug('Sending special message to Discord', text, channel, '->', `#${discordChannel.name}`);
        await discordChannel.send(text);
    }
     ...
     */
    executeIRCCommand(command, ...args) {
        logger_1.logger.info(`Executing IRC command: ${command} ${args.join(' ')}`);
        this.ircClient.send(command, ...args);
    }
    /**
     * Send raw IRC message (DANGEROUS: for internal use or trusted input only)
     */
    sendRawIRC(rawMessage) {
        ...
        if (sanitizedMessage !== rawMessage) {
            logger_1.logger.warn(`Attempted to send raw IRC message with newlines, stripping them: ${rawMessage}`);
        }
        if (!sanitizedMessage) {
            logger_1.logger.warn('Attempted to send an empty raw IRC message.');
            return;
        }
        logger_1.logger.info(`Sending raw IRC message: ${sanitizedMessage}`);
        this.ircClient.conn?.write(`${sanitizedMessage}\r\n`);
    }
    /**
     * Join an IRC channel
     */
    joinIRCChannel(channel, key) {
        logger_1.logger.info(`Joining IRC channel: ${channel}${key ? ' (with key)' : ''}`);
        if (key) {
     ...
     */
    partIRCChannel(channel, message) {
        logger_1.logger.info(`Parting IRC channel: ${channel}${message ? ` with message: ${message}` : ''}`);
        if (message) {
        ...
        this.ircHealthCheckInterval = setInterval(() => {
            void (async () => {
                ...
                if (health.connected && health.timeSinceActivity > staleThreshold) {
                    logger_1.logger.warn(`⚠️  IRC connection may be stale - no activity for ${Math.round(health.timeSinceActivity / 1000)}s`);
                }
                ...
                if (!health.connected) {
                    ...
                    if (isServiceAvailable && !this.ircReconnecting) {
                        logger_1.logger.info('🔄 IRC is down but circuit breaker is reset - attempting auto-reconnect');
                        try {
                            const success = await this.reconnectIRC();
                            if (success) {
                                logger_1.logger.info('✅ Auto-reconnect successful');
                            }
                            else {
                                logger_1.logger.warn('⚠️  Auto-reconnect failed, will retry on next health check');
                            }
                        }
                        catch (error) {
                            logger_1.logger.error('❌ Auto-reconnect error:', error);
                        }
                    }
                    else if (!isServiceAvailable) {
                        // Only log occasionally to avoid spam (circuit breaker timeout is 5 min)
                        logger_1.logger.debug('⚠️  IRC connection is down (circuit breaker active, will reset and retry)');
                    }
                }
            })();
     ...
     */
    sendIRCConnectionNotification(status, reason, attempt, maxAttempts) {
        ...
        if (channelMappingEntries.length === 0) {
            logger_1.logger.debug('No Discord channels mapped, skipping IRC connection notification');
            return;
        }
            ...
            if (discordChannel && isTextChannel(discordChannel)) {
                ...
                if (status === 'connected') {
                    this.statusNotifications.sendIRCConnectedNotification(textChannel)
                        .catch(error => logger_1.logger.error('Failed to send IRC connected notification:', error));
                }
                else if (status === 'disconnected') {
                    this.statusNotifications.sendIRCDisconnectedNotification(reason || 'Unknown reason', textChannel)
                        .catch(error => logger_1.logger.error('Failed to send IRC disconnected notification:', error));
                }
                    ...
                    this.statusNotifications.sendIRCReconnectingNotification(attempt, maxAttempts, textChannel)
                        .catch(error => logger_1.logger.error('Failed to send IRC reconnecting notification:', error));
                }
    ...
    async findOrCreatePmThread(ircNick) {
        ...
        if (cachedThreadId) {
            try {
                ...
                if (cachedThread && cachedThread.isThread()) {
                    ...
                    if (cachedThread.archived) {
                        try {
                            ...
                            logger_1.logger.warn('Failed to unarchive PM thread:', error);
                        }
                    }
                ...
                logger_1.logger.debug(`Cached thread ${cachedThreadId} not found, will search in persistence`);
        ...
        if (persistedThread) {
            try {
                ...
                if (thread && thread.isThread()) {
                    ...
                    if (thread.archived) {
                        try {
                            ...
                            logger_1.logger.warn('Failed to unarchive persisted PM thread:', error);
                        }
                    }
                ...
                logger_1.logger.debug(`Persisted thread ${persistedThread.threadId} not found, will try to create new one`);
        ...
        if (!pmChannel) {
            logger_1.logger.debug(`No PM thread exists for ${ircNick} and PM channel not configured`);
            return null;
        }
        // Search for existing thread by name
        const existingThread = pmChannel.threads.cache.find(thread => thread.name === threadName);
        if (existingThread) {
            ...
            if (existingThread.archived) {
                try {
                    ...
                    logger_1.logger.warn('Failed to unarchive existing PM thread:', error);
                }
            }
        ...
        try {
            ...
            logger_1.logger.debug(`Created new PM thread for ${ircNick}: ${newThread.id}`);
            // Record PM thread creation metrics
            this.metrics.recordPMThreadCreated();
            // Send initial message explaining the thread
            await newThread.send(`🔗 **Private message thread with IRC user \`${ircNick}\`**\n` +
                `Messages sent here will be forwarded to ${ircNick} on IRC.\n` +
                `Messages from ${ircNick} will appear in this thread.`);
            return newThread;
        }
        catch (error) {
            logger_1.logger.error('Failed to create PM thread:', error);
            return null;
        }
    }
    async handleIrcPrivateMessage(from, text) {
        // Check if user is ignored
        if (this.ignoredIrcUser(from)) {
            logger_1.logger.debug(`Ignoring PM from ignored IRC user: ${from}`);
            return;
        }
        logger_1.logger.debug(`Received IRC PM from ${from}: ${text}`);
        try {
            const thread = await this.findOrCreatePmThread(from);
            if (!thread) {
                logger_1.logger.debug(`No PM thread exists for ${from}, skipping message`);
                return;
            }
            ...
            logger_1.logger.debug(`Sent IRC PM to Discord thread: ${from} -> ${thread.name}`);
            // Record PM message metrics
            this.metrics.recordPMMessage();
        }
        catch (error) {
            logger_1.logger.error('Error handling IRC private message:', error);
        }
    }
    async handleDiscordPrivateMessage(message) {
        ...
        if (!threadName.startsWith(this.pmThreadPrefix))
            return false;
        ...
        if (this.ignoredDiscordUser(message.author)) {
            logger_1.logger.debug(`Ignoring PM from ignored Discord user: ${message.author.username}`);
            return true;
        }
        try {
                    ...
                    for (const [, attachment] of message.attachments) {
                        ...
                        if (this.s3Uploader) {
                            try {
                                ...
                                if (s3Url) {
                                    attachmentURL = s3Url;
                                    logger_1.logger.debug('Using S3 URL for PM attachment:', attachment.name);
                                }
                                else {
                                    logger_1.logger.debug('S3 upload failed, using Discord URL for PM attachment:', attachment.name);
                                }
                            }
                            catch (error) {
                                logger_1.logger.warn('S3 upload error for PM attachment, using Discord URL:', error);
                            }
                        }
                        const attachmentMessage = `[Attachment: ${attachment.name}] ${attachmentURL}`;
                        this.ircClient.say(ircNick, attachmentMessage);
                        logger_1.logger.debug(`Sent Discord attachment to IRC PM: ${message.author.username} -> ${ircNick}`);
                    }
                }
                return true;
            }
            // Format message for IRC (remove Discord formatting)
            text = (0, formatting_1.formatFromDiscordToIRC)(text);
            // Send to IRC user
            const lines = text.split('\n');
            for (const line of lines) {
                if (line.trim()) {
                    this.ircClient.say(ircNick, line);
                    logger_1.logger.debug(`Sent Discord PM to IRC: ${message.author.username} -> ${ircNick}: ${line}`);
                }
            }
            ...
            logger_1.logger.error('Error sending Discord PM to IRC:', error);
            return true; // Still handled, just failed
        }
    }
    async updatePmThreadForNickChange(oldNick, newNick) {
        ...
        try {
            ...
            if (pmChannel) {
                ...
                if (thread) {
                    ...
                    logger_1.logger.debug(`Updated PM thread name for nick change: ${oldNick} -> ${newNick}`);
                }
            }
        }
        catch (error) {
            logger_1.logger.warn('Failed to update PM thread name for nick change:', error);
        }
    }
    async saveChannelUsersToPersistence(channel) {
        try {
            ...
            logger_1.logger.error('Failed to save channel users to persistence:', error);
        }
    }
    /**
    ...
    loadS3Config(options = {}) {
        ...
        const validation = s3_uploader_1.S3Uploader.validateConfig(config);
        if (!validation.valid) {
            if (Object.keys(options).length > 0 || process.env.S3_REGION || process.env.S3_BUCKET) {
                logger_1.logger.warn('S3 configuration incomplete, S3 uploads disabled:', validation.errors);
            }
            else {
                logger_1.logger.debug('No S3 configuration provided, S3 uploads disabled');
            }
            return null;
        }
        logger_1.logger.info('S3 configuration loaded successfully', {
     ...
     */
    async uploadAttachmentToS3(attachment, userId, customFilename) {
        if (!this.s3Uploader) {
            return null;
        }
        try {
            ...
            if (!this.s3Uploader.isSupportedFileType(attachment.name || 'unknown')) {
                logger_1.logger.debug('Unsupported file type for S3 upload:', attachment.name);
                return null;
            }
            ...
            // Upload to S3
            const result = await this.s3Uploader.uploadFile(userId, buffer, attachment.name || 'unknown', customFilename, attachment.contentType || undefined);
            if (result.success && result.url) {
                logger_1.logger.info('Attachment uploaded to S3', {
                ...
                logger_1.logger.error('S3 upload failed:', result.error);
                return null;
            }
        }
        catch (error) {
            logger_1.logger.error('Failed to upload attachment to S3:', error);
            return null;
        }
    }
    /**
...
exports.TEST_HACK_CHANNEL = Symbol();
const isTextChannel = (channel) => channel.type === 'GUILD_TEXT' || exports.TEST_HACK_CHANNEL in channel;
exports.default = Bot;
