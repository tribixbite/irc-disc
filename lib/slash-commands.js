...
exports.slashCommands = exports.ircChannelDiscoveryCommand = exports.ircListsCommand = exports.ircCommandCommand = exports.ircWhoCommand = exports.ircChannelInfoCommand = exports.ircUserInfoCommand = exports.statusNotificationCommand = exports.mentionCommand = exports.s3Command = exports.recoveryCommand = exports.metricsCommand = exports.rateLimitCommand = exports.reconnectCommand = exports.directPmCommand = exports.pmCommand = exports.usersCommand = exports.statusCommand = void 0;
...
const s3RateLimiter = (0, s3_rate_limiter_1.createDefaultS3RateLimiter)();
...
function hasAdminPermission(interaction) {
    ...
    if (typeof interaction.member.permissions === 'string')
        return false;
    return interaction.member.permissions.has(discord_js_1.Permissions.FLAGS.ADMINISTRATOR);
}
...
exports.statusCommand = {
    data: {
        ...
        defaultMemberPermissions: discord_js_1.Permissions.FLAGS.ADMINISTRATOR,
    },
        ...
        try {
            ...
            if (bot.persistence) {
                try {
                    ...
                    logger_1.logger.warn('Failed to get uptime metric:', error);
                }
            }
        ...
        try {
            ...
            if (channelName) {
                ...
                if (users && users.size > 0) {
                    ...
                    embed.addField(`${fullChannelName} (${users.size} users)`, userList.length > 1024 ? userList.substring(0, 1021) + '...' : userList);
                    ...
                    if (channelInfo) {
                        ...
                        if (operatorCount > 0 || voicedCount > 0) {
                            let statusInfo = '';
                            if (operatorCount > 0)
                                statusInfo += `👑 ${operatorCount} operators`;
                            if (voicedCount > 0) {
                                if (statusInfo)
                                    statusInfo += ', ';
                    ...
                    embed.addField(fullChannelName, 'No users tracked (channel not found or not joined)');
                }
            }
            else {
            ...
            logger_1.logger.error('Error in users command:', error);
            await interaction.reply({
...
exports.pmCommand = {
    ...
    async execute(interaction, bot) {
        ...
        try {
            switch (subcommand) {
                ...
                case 'close': {
                    ...
                    try {
                        ...
                        logger_1.logger.warn('Failed to archive thread:', error);
                    }
                    await interaction.reply({
            ...
            logger_1.logger.error('Error in PM command:', error);
            await interaction.reply({
        ...
        try {
                ...
                try {
                    ...
                    if (fetched?.isThread()) {
                        ...
                        if (thread.archived) {
                            ...
                            logger_1.logger.info(`Unarchived PM thread for ${nickname}`);
                        }
                    }
                    else {
                    ...
                    logger_1.logger.warn(`Cleaning up stale PM thread for ${nickname}:`, fetchError);
            ...
            if (!thread) {
                try {
                    ...
                    logger_1.logger.info(`Created new PM thread for ${nickname}: ${thread.id}`);
                    bot.metrics.recordPMThreadCreated();
                }
                catch (createError) {
                    logger_1.logger.error(`Failed to create PM thread for ${nickname}:`, createError);
                    await interaction.editReply({
                        content: `❌ Failed to create thread: ${createError.message}`
                    });
                    return;
                }
            }
            ...
            if (message && thread) {
                try {
                    ...
                    logger_1.logger.debug(`Sent initial PM to ${nickname}: ${message.substring(0, 50)}...`);
                }
                catch (sendError) {
                    logger_1.logger.error(`Failed to send initial message to ${nickname}:`, sendError);
                    await thread.send(`⚠️ **Failed to send message to IRC:** ${sendError.message}`);
                }
            }
        }
        catch (error) {
            logger_1.logger.error('Error in direct PM command:', error);
            await interaction.editReply({
                content: '❌ Failed to execute PM command.'
            });
        }
    ...
    async execute(interaction, bot) {
        ...
        try {
            ...
            bot.ircClient.disconnect('Manual reconnect requested', () => {
                logger_1.logger.info('IRC client disconnected for manual reconnection');
            ...
            logger_1.logger.error('Error in reconnect command:', error);
...
exports.rateLimitCommand = {
    ...
    async execute(interaction, bot) {
        ...
        try {
            ...
            logger_1.logger.error('Error in rate limit command:', error);
            await interaction.reply({
        ...
        try {
            ...
            switch (subcommand) {
                ...
                case 'export': {
                    ...
                    const attachment = new discord_js_1.MessageAttachment(buffer, `irc-bridge-metrics-${Date.now()}.txt`);
                    await interaction.reply({
...
exports.recoveryCommand = {
    ...
    async execute(interaction, bot) {
        ...
        try {
            ...
            switch (subcommand) {
                ...
                case 'force': {
                    ...
                    try {
                        ...
                        await interaction.editReply({
                            content: `❌ **Manual Recovery Failed**\n\nFailed to force recovery for ${service}: ${error.message}`
                        });
                    }
                    break;
                }
            ...
            logger_1.logger.error('Error in recovery command:', error);
            await interaction.reply({
...
async function handleS3ConfigCommands(interaction, bot, subcommand) {
    ...
    switch (subcommand) {
        case 'set': {
            ...
            try {
                const s3Config = { guildId, bucket, region, endpoint, accessKeyId, secretAccessKey, keyPrefix, publicUrlBase: undefined, forcePathStyle: !!endpoint, maxFileSizeMb, allowedRoles: undefined, createdAt: Date.now(), updatedAt: Date.now() };
                if (!bot.persistence)
                ...
                const testUploader = new s3_uploader_1.S3Uploader({ region, bucket, accessKeyId, secretAccessKey, endpoint, keyPrefix, forcePathStyle: s3Config.forcePathStyle });
                ...
                await interaction.editReply({ content: `❌ Save failed: ${error.message}` });
            }
            break;
        }
        case 'view': {
            ...
            const embed = new discord_js_1.MessageEmbed().setTitle('🔧 S3 Configuration').setColor('#3498db')
            ...
            if (config.endpoint)
                embed.addField('Endpoint', config.endpoint, false);
            if (config.keyPrefix)
                embed.addField('Prefix', config.keyPrefix, true);
        ...
        case 'test': {
            ...
            const config = await bot.persistence.getS3Config(guildId);
            if (!config) {
                await interaction.editReply({ content: '❌ Use `/s3 config set` first.' });
                return;
            }
            const uploader = new s3_uploader_1.S3Uploader({ region: config.region, bucket: config.bucket, accessKeyId: config.accessKeyId, secretAccessKey: config.secretAccessKey, endpoint: config.endpoint, keyPrefix: config.keyPrefix, forcePathStyle: config.forcePathStyle });
...
async function handleS3FilesCommands(interaction, bot, subcommand) {
    const guildId = interaction.guildId;
    if (!bot.persistence) {
    ...
    const config = await bot.persistence.getS3Config(guildId);
    if (!config) {
        await interaction.reply({ content: '❌ **Not Configured**\n\nUse `/s3 config set` first.', ephemeral: true });
        return;
    }
    const uploader = new s3_uploader_1.S3Uploader({ region: config.region, bucket: config.bucket, accessKeyId: config.accessKeyId, secretAccessKey: config.secretAccessKey, endpoint: config.endpoint, keyPrefix: config.keyPrefix, publicUrlBase: config.publicUrlBase, forcePathStyle: config.forcePathStyle });
    switch (subcommand) {
        case 'upload': {
            await interaction.deferReply({ ephemeral: true });
            // Check rate limit
            ...
            try {
                ...
                if (result.success) {
                    const embed = new discord_js_1.MessageEmbed().setTitle('✅ Upload Success').setColor('#00ff00')
        ...
        case 'list': {
            ...
            try {
                ...
                const embed = new discord_js_1.MessageEmbed().setTitle(`📁 Files in ${config.bucket}`).setColor('#3498db').setTimestamp();
                if (prefix)
                ...
                await interaction.editReply({ content: `❌ List failed: ${error.message}` });
            }
            break;
        }
        case 'info': {
            ...
            try {
                ...
                await interaction.editReply({ content: `❌ Failed to get file info: ${error.message}` });
            }
            break;
        }
        case 'rename': {
            ...
            try {
                ...
                await interaction.editReply({ content: `❌ Rename failed: ${error.message}` });
            }
            break;
        }
        case 'delete': {
            ...
            try {
                ...
                const confirmButton = new discord_js_1.MessageButton()
                    .setCustomId(`s3_delete_confirm_${key}`)
                    .setLabel('Delete File')
                    .setStyle('DANGER');
                const cancelButton = new discord_js_1.MessageButton()
                    .setCustomId('s3_delete_cancel')
                    .setLabel('Cancel')
                    .setStyle('SECONDARY');
                const row = new discord_js_1.MessageActionRow().addComponents(confirmButton, cancelButton);
                    ...
                    if (buttonInteraction.customId === 's3_delete_cancel') {
                        ...
                        try {
                            ...
                            const errorEmbed = new discord_js_1.MessageEmbed()
                                ...
                                .setColor('#ff0000')
                                .setDescription(deleteError.message)
                                .setTimestamp();
...
async function handleS3StatusCommand(interaction, bot) {
    await interaction.deferReply({ ephemeral: true });
    const embed = new discord_js_1.MessageEmbed().setTitle('📊 S3 Storage Status').setColor('#3498db').setTimestamp();
    if (!bot.persistence) {
...
async function handleS3ShareCommand(interaction, bot) {
    await interaction.deferReply({ ephemeral: true });
    const guildId = interaction.guildId;
    if (!bot.persistence) {
        await interaction.editReply({ content: '❌ Database not available.' });
    ...
    try {
        // Upload to S3
        const uploader = new s3_uploader_1.S3Uploader({
        ...
        // Build share embed
        const shareEmbed = new discord_js_1.MessageEmbed()
        ...
        logger_1.logger.error('S3 share command error:', error);
        await interaction.editReply({ content: `❌ Share failed: ${error.message}` });
    }
}
    ...
    async execute(interaction, bot) {
        ...
        try {
            ...
            if (subcommandGroup === 'config')
                await handleS3ConfigCommands(interaction, bot, subcommand);
            else if (subcommandGroup === 'files')
                await handleS3FilesCommands(interaction, bot, subcommand);
            else if (subcommand === 'share')
                await handleS3ShareCommand(interaction, bot);
            else if (subcommand === 'status')
                await handleS3StatusCommand(interaction, bot);
        }
        catch (error) {
            logger_1.logger.error('S3 command error:', error);
            try {
                const method = interaction.deferred || interaction.replied ? 'editReply' : 'reply';
                await interaction[method]({ content: `❌ Error: ${error.message}`, ephemeral: true });
            }
            catch (replyError) {
                logger_1.logger.error('Failed to send error:', replyError);
            }
        }
    }
};
...
exports.mentionCommand = {
    ...
    async execute(interaction, bot) {
        ...
        try {
            ...
            logger_1.logger.error('Error in mention command:', error);
            await interaction.reply({
...
exports.statusNotificationCommand = {
    data: {
    ...
    async execute(interaction, bot) {
        ...
        try {
            ...
            switch (subcommand) {
                ...
                case 'test': {
                    const notificationType = interaction.options.getString('type', true);
            ...
            logger_1.logger.error('Error in status notification command:', error);
            await interaction.reply({
...
exports.ircUserInfoCommand = {
    ...
    async execute(interaction, bot) {
        ...
        try {
            ...
            switch (subcommand) {
                case 'lookup': {
                    ...
                    if (userInfo.realname)
                        embed.addField('Real Name', userInfo.realname, true);
                    ...
                    if (userInfo.server)
                        embed.addField('IRC Server', userInfo.server, true);
                    // Account and security
                    if (userInfo.account)
                        embed.addField('Services Account', userInfo.account, true);
                ...
                case 'search': {
                    ...
                    if (nick)
                        searchCriteria.nick = nick;
                    if (hostname)
                        searchCriteria.hostname = hostname;
                    if (realname)
                        searchCriteria.realname = realname;
                    if (channel)
                        searchCriteria.channel = channel;
                    if (operatorsOnly)
                        searchCriteria.isOperator = true;
                    if (secureOnly)
                        searchCriteria.isSecure = true;
            ...
            logger_1.logger.error('Error in IRC user info command:', error);
            await interaction.reply({
...
exports.ircChannelInfoCommand = {
    ...
    async execute(interaction, bot) {
        try {
            ...
            logger_1.logger.error('Error in IRC channel info command:', error);
            ...
            try {
                ...
                logger_1.logger.warn('Could not send error reply for IRC channel info command (interaction may have expired):', replyError);
            }
        }
    }
};
// IRC WHO command
exports.ircWhoCommand = {
    data: {
        name: 'irc-who',
        ...
        try {
            ...
            try {
                ...
                const whoWithTimeout = Promise.race([
                    ...
                    new Promise((_, reject) => setTimeout(() => reject(new Error('IRC WHO query timed out after 30s')), 30000))
                ]);
                ...
                await interaction.editReply({
                    content: `❌ **WHO Query Failed**\n\nError executing WHO query for pattern \`${pattern}\`: ${error.message}`
                });
            }
        }
        catch (error) {
            logger_1.logger.error('Error in IRC WHO command:', error);
...
exports.ircCommandCommand = {
    ...
    async execute(interaction, bot) {
        ...
        try {
            ...
            logger_1.logger.error('Error in IRC command execution:', error);
            await interaction.reply({
    ...
    async execute(interaction, bot) {
        ...
        try {
            ...
            try {
                ...
                await interaction.editReply({
                    content: `✅ **${listNames[listType]} Query Sent**\n\nRequested ${listNames[listType].toLowerCase()} for ${channelName}.\n\n⚠️ **Note**: Results will appear in IRC client logs. Full list viewing in Discord will be implemented in a future update.`
                });
            }
            catch (error) {
                await interaction.editReply({
                    content: `❌ **List Query Failed**\n\nError querying ${listType} list for ${channelName}: ${error.message}`
                });
            }
        }
        catch (error) {
            logger_1.logger.error('Error in IRC lists command:', error);
            if (interaction.deferred) {
                await interaction.editReply({
...
exports.ircChannelDiscoveryCommand = {
    data: {
        name: 'irc-channels',
    ...
    async execute(interaction, bot) {
        ...
        try {
            ...
            switch (subcommand) {
                case 'list': {
                    ...
                    try {
                        ...
                        const listChannelsWithTimeout = Promise.race([
                            ...
                            new Promise((_, reject) => setTimeout(() => reject(new Error('IRC channel list request timed out after 30s')), 30000))
                        ]);
                        ...
                        for (let i = 1; i < channelChunks.length && i < 5; i++) { // Limit to 5 total messages
                            ...
                            const chunkEmbed = new discord_js_1.MessageEmbed()
                        ...
                        logger_1.logger.error('Error listing IRC channels:', error);
                        await interaction.editReply({
                ...
                case 'join': {
                    ...
                    try {
                        ...
                        logger_1.logger.error('Error joining IRC channel:', error);
                        await interaction.editReply({
                ...
                case 'part': {
                    ...
                    try {
                        ...
                        logger_1.logger.error('Error leaving IRC channel:', error);
                        await interaction.editReply({
            ...
            logger_1.logger.error('Error in IRC channel discovery command:', error);
            ...
            try {
                ...
                logger_1.logger.error('Failed to send error message to user (interaction may have expired):', replyError);
            }
        }
    }
};
...
async function registerSlashCommands(bot) {
    try {
        if (!bot.discord.application) {
            logger_1.logger.error('Discord application not available for command registration');
            return;
        }
        const commandData = exports.slashCommands.map(command => command.data);
        ...
        if (guilds.size > 0) {
            ...
            for (const [guildId, guild] of guilds) {
                try {
                    ...
                    logger_1.logger.info(`Registered ${exports.slashCommands.length} slash commands to guild: ${guild.name} (${guildId})`);
                    successCount++;
                }
                catch (guildError) {
                    logger_1.logger.error(`Failed to register commands to guild ${guild.name}:`, guildError);
                }
            }
            if (successCount > 0) {
                logger_1.logger.info(`✅ Successfully registered slash commands to ${successCount} guild(s)`);
            }
            else {
                logger_1.logger.warn('Failed to register commands to any guild, falling back to global');
                await bot.discord.application.commands.set(commandData);
                logger_1.logger.info(`Registered ${exports.slashCommands.length} slash commands globally (may take up to 1 hour to appear)`);
            }
        }
        else {
...
async function handleSlashCommand(interaction, bot) {
    const command = exports.slashCommands.find(cmd => cmd.data.name === interaction.commandName);
    if (!command) {
        await interaction.reply({
    ...
    try {
        ...
        logger_1.logger.error(`Error executing slash command ${interaction.commandName}:`, error);
...
async function handleS3ListPagination(interaction, bot) {
    ...
    try {
        ...
        logger_1.logger.error('Error handling S3 list pagination:', error);
        await interaction.editReply({
            content: `❌ Pagination failed: ${error.message}`,
            components: []
        });
    }
}
