...
class MessageSynchronizer {
    ...
    editWindow = 5 * 60 * 1000; // 5 minutes edit window
    constructor(bot) {
        this.bot = bot;
    }
    /**
     ...
     */
    recordMessage(discordMessageId, ircChannel, ircMessage, author) {
        const record = {
            discordMessageId,
            ircChannel,
            ircMessage,
        ...
        logger_1.logger.debug(`Recorded message ${discordMessageId} -> ${ircChannel}: ${ircMessage.substring(0, 50)}...`);
    }
    /**
        ...
        try {
            ...
            if (newMessage.partial) {
                try {
                    ...
                    logger_1.logger.warn(`Failed to fetch full message for edit ${newMessage.id}:`, error);
                    return;
                }
            }
            else {
            ...
            if (!messageRecord) {
                ...
                logger_1.logger.debug(`No record found for edited message ${fullMessage.id} (possible race condition)`);
                return;
            }
            ...
            if (timeSinceOriginal > this.editWindow) {
                logger_1.logger.debug(`Edit window expired for message ${fullMessage.id} (${timeSinceOriginal}ms ago)`);
                return;
            }
            ...
            try {
                ...
                logger_1.logger.warn(`Failed to parse edited message content for ${fullMessage.id}:`, error);
                return;
            }
            if (!formattedContent || formattedContent.trim() === '') {
                logger_1.logger.debug(`Skipping edit with empty content for message ${fullMessage.id}`);
                return;
            }
            // Check if content actually changed
            if (formattedContent === messageRecord.ircMessage) {
                logger_1.logger.debug(`No content change detected for message ${fullMessage.id}`);
                return;
            }
            ...
            if (this.bot.ircClient && this.bot.ircClient.readyState === 'open') {
                try {
                    ...
                    logger_1.logger.info(`Sent edit notification to ${messageRecord.ircChannel}: ${messageRecord.author} edited message`);
                    // Record edit metrics
                    this.bot.metrics.recordEdit();
                }
                catch (error) {
                    logger_1.logger.warn(`Failed to send edit notification (IRC may have disconnected):`, error);
                ...
                logger_1.logger.warn('IRC client not ready, cannot send edit notification');
            }
        }
        catch (error) {
            logger_1.logger.error('Error handling message edit:', error);
        }
    }
    /**
    ...
    async handleMessageDelete(message) {
        try {
            ...
            if (!messageRecord) {
                ...
                logger_1.logger.debug(`No record found for deleted message ${message.id} (possible race condition)`);
                return;
            }
            // Check if deletion is within the allowed window
            const timeSinceOriginal = Date.now() - messageRecord.timestamp;
            if (timeSinceOriginal > this.editWindow) {
                logger_1.logger.debug(`Delete window expired for message ${message.id} (${timeSinceOriginal}ms ago)`);
            ...
            if (this.bot.ircClient && this.bot.ircClient.readyState === 'open') {
                try {
                    ...
                    logger_1.logger.info(`Sent delete notification to ${messageRecord.ircChannel}: ${messageRecord.author} deleted message`);
                    // Record delete metrics
                    this.bot.metrics.recordDelete();
                }
                catch (error) {
                    logger_1.logger.warn(`Failed to send delete notification (IRC may have disconnected):`, error);
                    // Continue to remove from history even if send failed
                }
            }
            else {
                logger_1.logger.warn('IRC client not ready, cannot send delete notification');
            }
            // Remove from history (even if send failed)
            this.messageHistory.delete(message.id);
        }
        catch (error) {
            logger_1.logger.error('Error handling message deletion:', error);
        }
    }
    /**
    ...
    async handleBulkDelete(messages) {
        try {
            ...
            if (relevantMessages === 0) {
                logger_1.logger.debug(`Bulk delete of ${deletedCount} messages, none were tracked`);
                return;
            }
            ...
            for (const channel of channels) {
                ...
                if (this.bot.ircClient && this.bot.ircClient.readyState === 'open') {
                    try {
                        ...
                        logger_1.logger.info(`Sent bulk delete notification to ${channel}: ${relevantMessages} messages`);
                    }
                    catch (error) {
                        logger_1.logger.warn(`Failed to send bulk delete notification to ${channel} (IRC may have disconnected):`, error);
            ...
            logger_1.logger.error('Error handling bulk message deletion:', error);
        }
    }
    /**
    ...
    cleanupOldMessages() {
        ...
        if (cleanedCount > 0) {
            logger_1.logger.debug(`Cleaned up ${cleanedCount} old message records`);
        }
        ...
        if (this.messageHistory.size > this.maxHistorySize) {
            ...
            logger_1.logger.debug(`Removed ${toRemove} oldest message records to maintain size limit`);
        }
    }
    /**
    ...
    clearHistory() {
        ...
        logger_1.logger.info(`Cleared ${count} message records from history`);
    }
    /**
     * Save message history to persistence (if enabled)
     */
    async saveHistoryToPersistence() {
        if (!this.bot.persistence)
            return;
        try {
            ...
            logger_1.logger.debug('Saved message history to persistence');
        }
        catch (error) {
            logger_1.logger.error('Failed to save message history to persistence:', error);
        }
    }
    /**
     * Load message history from persistence (if available)
     */
    async loadHistoryFromPersistence() {
        if (!this.bot.persistence)
            return;
        try {
            ...
            if (historyData) {
                ...
                this.cleanupOldMessages();
                logger_1.logger.info(`Loaded ${this.messageHistory.size} message records from persistence`);
            }
        }
        catch (error) {
            logger_1.logger.error('Failed to load message history from persistence:', error);
